      ...
      <div className="sc-metrics-grid">
        {metrics.map((x, i) => (
          <div key={x.v} style={{ paddingLeft: i === 0 ? 0 : 24, borderLeft: i === 0 ? "none" : "1px solid #ececec" }}>
            <div style={{ fontFamily: "'ESKlarheit',sans-serif", fontWeight: 800, fontSize: "clamp(32px,4vw,44px)", letterSpacing: "-0.03em", color: "#0f3d10", lineHeight: 1, marginBottom: 10 }}>
              {x.k}
            </div>
            <div style={{ fontSize: 13, color: "#4a4a4a", fontWeight: 500, fontFamily: "'Gilroy',sans-serif" }}>{x.v}</div>
          </div>
        ))}
      </div>
    </section>
